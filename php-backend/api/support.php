...
try {
    switch ($action) {
        ...
        case 'send-message':
            ...
            $senderEmail = $user['email'] ?? ($guestEmail ?: ($visitorToken ? "visitor-{$visitorToken}@guest.local" : 'guest@antiprofiles.com'));
            ...
            if (!$conv && $visitorToken) {
                $stmt = $db->prepare("SELECT * FROM support_conversations WHERE visitor_token = ? AND status != 'closed' ORDER BY COALESCE(last_message_at, created_at) DESC LIMIT 1 FOR UPDATE");
                $stmt->execute([$visitorToken]);
                $conv = $stmt->fetch();
            }
            ...
            if (!$conv) {
                ...
                $insertConv->execute([
                    ...
                    $userId,
                    $visitorToken,
                    $guestName ?: ($user['name'] ?? 'Guest Visitor'),
            ...
            // Check Idempotency via client_message_id
            if ($clientMsgId) {
                $existingStmt = $db->prepare("SELECT * FROM support_messages WHERE conversation_id = ? AND client_message_id = ?");
                $existingStmt->execute([$convId, $clientMsgId]);
                $existingMsg = $existingStmt->fetch();
                if ($existingMsg) {
                    $db->commit();
                    respondJson([
                        'success' => true,
            ...
            $msgStmt = $db->prepare("
                INSERT INTO support_messages (id, conversation_id, client_message_id, sender_id, sender_name, sender_type, message, message_type, status, is_read, created_at)
            ...
            logSecurityEvent('CHAT_MESSAGE_SAVED', 'info', "Support message [{$msgId}] saved in conversation [{$convId}] from {$senderName}");
            ...
            // Auto-Reply on brand new conversation if enabled in settings
                ...
                try {
                    ...
                    if ($autoEnabled && ($autoEnabled['value'] === 'true' || $autoEnabled['value'] === '1') && $autoRow && !empty($autoRow['value'])) {
                        ...
                        $db->prepare("
                            ...
                            VALUES (?, ?, 'agent_system', 'Support Team', 'agent', ?, 'text', 'sent', 1, DATE_ADD(NOW(), INTERVAL 1 SECOND))
        ...
        case 'get-messages':
            ...
            if (!$convId) {
                if ($user) {
                    $stmt = $db->prepare("SELECT id FROM support_conversations WHERE user_id = ? ORDER BY COALESCE(last_message_at, created_at) DESC LIMIT 1");
                    $stmt->execute([$user['id']]);
                    $c = $stmt->fetch();
                    $convId = $c['id'] ?? null;
                } elseif ($visitorToken) {
                    $stmt = $db->prepare("SELECT id FROM support_conversations WHERE visitor_token = ? ORDER BY COALESCE(last_message_at, created_at) DESC LIMIT 1");
                    $stmt->execute([$visitorToken]);
                    $c = $stmt->fetch();
                    $convId = $c['id'] ?? null;
                }
        ...
        case 'admin-conversations':
            ...
            $stmt = $db->prepare("
                SELECT 
                    ...
                    COALESCE((SELECT p.name FROM subscriptions s JOIN pricing_plans p ON s.plan_id = p.id WHERE s.user_id = c.user_id ORDER BY s.created_at DESC LIMIT 1), 'Free / Guest') as plan_name,
        ...
        case 'admin-send-reply':
            ...
            logSecurityEvent('CHAT_ADMIN_REPLY_SENT', 'info', "Admin reply [{$msgId}] delivered to conversation [{$convId}]");
    ...
    logSecurityEvent('CHAT_MESSAGE_SAVE_FAILED', 'warning', 'Chat API Exception: ' . $e->getMessage());
    respondJson(['success' => false, 'error' => 'Chat server error: ' . $e->getMessage()], 500);
}

...
export function setupAuthIPC(): void {
  ...
  // ── Google OAuth 2.0 PKCE System Browser Login Handler (RFC 8252) ──
  ipcMain.handle('auth:google-login', async () => {
    try {
      const { startGoogleSystemBrowserOAuth } = require('../security/google-oauth-loopback')
      const oauthResult = await startGoogleSystemBrowserOAuth()

      if (!oauthResult.success) {
        return { success: false, error: oauthResult.error || 'Google authentication failed.' }
      }

      const profile = oauthResult.userProfile
      if (profile && profile.email) {
        // Upsert user in local database
        let user = userRepo.getByEmail(profile.email)
        if (!user) {
        ...
        sessionManager.registerSession(sessionToken, user)
      ...
      logger.error('auth', `Google OAuth login failed: ${err.message}`)
      return { success: false, error: err.message }
    }
  })
}
